const offcanvasData = [
  ...
  {   
    ...
    offcanvasBody:`NodeJS is a server side runtime environment that executes JavaScript code outside of the browser. This means all backend service applications are written in JavaScript. NodeJS offers thousands of libraries made by developers for developers.\n
    Some of the reasons I had chosen NodeJS for my service applications is because I liked the "JavaScript everywhere" paradigm. I would have the freedom to use JavaScript as my client side and server side language. 
    `
  },
  {   
  ...
  {   
    ...
    offcanvasBody: 'MySQL is a popular relational database utilizing SQL. I mainly chose it since I was most familiar using it in my previous work.'
  },
  {   
    id: 'restAPI-offcanvas',
    offcanvasTitle: 'My Experience with Rest API',
    offcanvasBody: `A RESTful API conforms to the REST architecture. REST architechure says that the backend service performs HTTP requests on an endpoint. HTTP requests are performed by using methods such as POST to update a resource, GET to retrieve an individual resource or the whole resource, etc. Many people often implement these API with CRUD (create, retrieve, update, and delete) with HTTP requests as the methods to perform these actions on the given endpoint. REST API allow for more versatile data management than fully committing to just using SQL queries in a database table.`
  },
  {   
    id: 'stripe-offcanvas',
  ...
  {   
    ...
    offcanvasBody: `When I first developed the web application, I had no direction in terms of design. I used pure HTML and CSS to create a usable component and it quite frankly looked dull. My homepage is a testiment to this statement, as it was the first component I made. The first carousel on the homepage was made from scratch and is a little dull since CSS is not my strongest point. Comparing this to my Service or About Me page, the slideshows created from React-bootstrap's carousel (and the entire components) are a step up.\n    
    I really loved learning React-bootstrap and its flexbox design pattern with React-bootstrap's layout grid system. Moving away from the design aspect, I really enjoyed when my functionality came to life such as my cart being able to save user's items across browser tabs, having the item price and total price be updated by the quantity the customer wants for each item, and implementing Stripe's checkout session process.`
  },
  {   
    id: 'issues-offcanvas',
